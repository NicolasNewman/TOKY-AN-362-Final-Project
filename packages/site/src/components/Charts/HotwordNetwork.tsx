...
const HotwordNetwork: React.FC<IProps> = ({ network, focus, setFocus }) => {
	return (
		<NetworkChart
			data={
				focus && typeof focus === 'string'
					? (() => {
							...
							const temp: Network = {
								links: network.links.filter((l) => {
									...
									if (l.source === focus || l.target === focus) {
										maxLinkWeight = Math.max(maxLinkWeight, parseInt(l.weight));
										return true;
									}
									return false;
								}),
			...
			iterations={120}
			animate={false}
			linkDistance={(n) => shiftRange(parseInt(n.weight), [0, network.maxLinkWeight], [300, 200], Math.ceil)}
			linkColor={(link) => {
			...
			linkThickness={(link) =>
				shiftRange(parseInt(link.data.weight), [0, network.maxLinkWeight], [1, 12], Math.ceil)
			}
			nodeBorderWidth={1}
